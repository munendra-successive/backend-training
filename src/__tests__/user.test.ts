...
describe('POST /users/login', () => {
    ...
    it('should return status 200, have valid token', async () => {
        const res = await request(app)
            ...
            .set(
                'Authorization',
                'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6Im1vbnVAZ21haWwuY29tIiwicGFzc3dvcmQiOiJNT251QDEyMyIsImlhdCI6MTcwNDY5ODMyNywiZXhwIjoxNzA0NzAwMTI3fQ.PlzeJ8nk_brVY_EgJp6SFm8ttz-Z4TJlm6I0VNIW8q8',
            )
            .send({ email: 'monu@gmail.com', password: 'MOnu@123' });
        expect(res.status).toBe(200);
        expect(res.body).toHaveProperty('message');
